import { ImageResponse } from "next/og";

export const runtime = "edge";
export const alt = "AWS SBG VJIT Project";
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";

const PROJECT_ID =
  process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || "aws-sbg-vjit";
const REST_URL = `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/(default)/documents:runQuery`;

const MOCK_PROJECTS: Record<
  string,
  { title: string; tagline: string; stack: string[]; featured: boolean }
> = {
  "vjit-cloud-portal": {
    title: "VJIT Cloud Portal",
    tagline:
      "Centralized serverless student hub built using Next.js, API Gateway, and Lambda.",
    stack: ["Next.js", "AWS Lambda", "DynamoDB", "TailwindCSS"],
    featured: true,
  },
  "aws-cost-optimizer": {
    title: "AWS Cost Optimizer",
    tagline:
      "Automated tool that scans and stops idle EC2 and RDS instances on VJIT sandbox.",
    stack: ["Python", "AWS Lambda", "CloudWatch", "SNS"],
    featured: true,
  },
  "serverless-shortener": {
    title: "Serverless Shortener",
    tagline:
      "A sub-millisecond URL shortener powered by AWS CloudFront and DynamoDB Global Tables.",
    stack: ["CloudFront", "DynamoDB", "S3", "API Gateway"],
    featured: false,
  },
};

async function getProject(slug: string) {
  try {
    const res = await fetch(REST_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        structuredQuery: {
          from: [{ collectionId: "projects" }],
          where: {
            fieldFilter: {
              field: { fieldPath: "slug" },
              op: "EQUAL",
              value: { stringValue: slug },
            },
          },
          limit: 1,
        },
      }),
    });
    const data = await res.json();
    const doc = data?.[0]?.document;
    if (!doc) return null;
    const fields = doc.fields;

    // Parse stack array
    const values = fields?.stack?.arrayValue?.values || [];
    const stack = (values as Array<{ stringValue?: string }>)
      .map((v) => v.stringValue)
      .filter((v): v is string => typeof v === "string");

    return {
      title: fields?.title?.stringValue || "",
      tagline: fields?.tagline?.stringValue || "",
      stack,
      featured: fields?.featured?.booleanValue ?? false,
    };
  } catch (err) {
    // eslint-disable-next-line no-console
    console.warn("Failed to fetch project details via REST:", err);
    return null;
  }
}

export default async function Image({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const rawProject = await getProject(slug);

  const project = rawProject ||
    MOCK_PROJECTS[slug] || {
      title: slug
        .split("-")
        .map((w) => w.charAt(0).toUpperCase() + w.slice(1))
        .join(" "),
      tagline:
        "A serverless cloud application built and shipped by AWS SBG VJIT builders.",
      stack: ["AWS", "Serverless", "Cloud"],
      featured: false,
    };

  return new ImageResponse(
    <div
      style={{
        width: "100%",
        height: "100%",
        display: "flex",
        flexDirection: "column",
        justifyContent: "space-between",
        background: "linear-gradient(135deg, #0b0f17 0%, #05070a 100%)",
        padding: "60px 80px",
        fontFamily: "sans-serif",
        color: "#fafaf7",
        position: "relative",
        overflow: "hidden",
      }}
    >
      {/* Glow highlight bar at the top */}
      <div
        style={{
          position: "absolute",
          top: 0,
          left: 0,
          width: "100%",
          height: "6px",
          background: "linear-gradient(to right, #ff9900, #ff5500, #ff9900)",
        }}
      />

      {/* Decorative Technical Grid Overlay */}
      <div
        style={{
          position: "absolute",
          top: 0,
          left: "15%",
          width: "1px",
          height: "100%",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />
      <div
        style={{
          position: "absolute",
          top: 0,
          left: "30%",
          width: "1px",
          height: "100%",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />
      <div
        style={{
          position: "absolute",
          top: 0,
          left: "45%",
          width: "1px",
          height: "100%",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />
      <div
        style={{
          position: "absolute",
          top: 0,
          left: "60%",
          width: "1px",
          height: "100%",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />
      <div
        style={{
          position: "absolute",
          top: 0,
          left: "75%",
          width: "1px",
          height: "100%",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />
      <div
        style={{
          position: "absolute",
          top: 0,
          left: "90%",
          width: "1px",
          height: "100%",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />
      <div
        style={{
          position: "absolute",
          top: "20%",
          left: 0,
          width: "100%",
          height: "1px",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />
      <div
        style={{
          position: "absolute",
          top: "40%",
          left: 0,
          width: "100%",
          height: "1px",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />
      <div
        style={{
          position: "absolute",
          top: "60%",
          left: 0,
          width: "100%",
          height: "1px",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />
      <div
        style={{
          position: "absolute",
          top: "80%",
          left: 0,
          width: "100%",
          height: "1px",
          background: "rgba(255, 153, 0, 0.04)",
        }}
      />

      {/* Top Header Row with Logo */}
      <div
        style={{
          display: "flex",
          justifyContent: "space-between",
          alignItems: "center",
          width: "100%",
          zIndex: 10,
        }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <div
            style={{
              display: "flex",
              padding: "6px",
              background: "rgba(255, 153, 0, 0.08)",
              border: "1px solid rgba(255, 153, 0, 0.25)",
              borderRadius: "6px",
            }}
          >
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              stroke="#ff9900"
              strokeWidth="2.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            >
              <path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z" />
            </svg>
          </div>
          <div style={{ display: "flex", flexDirection: "column" }}>
            <div
              style={{
                display: "flex",
                fontSize: 18,
                fontWeight: 800,
                letterSpacing: "0.08em",
                textTransform: "uppercase",
                color: "#ff9900",
              }}
            >
              AWS Student Builder Group
            </div>
            <div
              style={{
                display: "flex",
                fontSize: 11,
                color: "#8b93a1",
                letterSpacing: "0.05em",
                textTransform: "uppercase",
                fontWeight: 600,
              }}
            >
              VJIT Chapter • Hyderabad
            </div>
          </div>
        </div>

        {/* Project Tag / Badge */}
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          {project.featured && (
            <div
              style={{
                display: "flex",
                background: "rgba(255, 153, 0, 0.08)",
                border: "1px solid rgba(255, 153, 0, 0.3)",
                borderRadius: "4px",
                padding: "6px 14px",
                fontSize: 13,
                fontWeight: 700,
                color: "#ff9900",
                textTransform: "uppercase",
                letterSpacing: "0.05em",
              }}
            >
              Featured
            </div>
          )}
          <div
            style={{
              display: "flex",
              background: "rgba(255, 255, 255, 0.03)",
              border: "1px solid rgba(255, 255, 255, 0.1)",
              borderRadius: "4px",
              padding: "6px 14px",
              fontSize: 13,
              fontWeight: 700,
              color: "#fafaf7",
              textTransform: "uppercase",
              letterSpacing: "0.05em",
            }}
          >
            Project
          </div>
        </div>
      </div>

      {/* Main Info Box inside glassmorphic panel */}
      <div
        style={{
          display: "flex",
          flexDirection: "column",
          gap: "14px",
          background: "rgba(11, 15, 23, 0.65)",
          border: "1px solid rgba(255, 255, 255, 0.05)",
          borderRadius: "16px",
          padding: "35px",
          margin: "20px 0",
          zIndex: 10,
        }}
      >
        <div
          style={{
            display: "flex",
            fontSize: 13,
            fontWeight: 700,
            letterSpacing: "0.2em",
            textTransform: "uppercase",
            color: "#ff9900",
          }}
        >
          Student Showcase
        </div>
        <div
          style={{
            display: "flex",
            fontSize: 48,
            fontWeight: 900,
            lineHeight: 1.15,
            color: "#ffffff",
            maxWidth: "950px",
          }}
        >
          {project.title}
        </div>
        <div
          style={{
            display: "flex",
            fontSize: 18,
            color: "#8b93a1",
            maxWidth: "800px",
            lineHeight: 1.4,
          }}
        >
          {project.tagline}
        </div>
      </div>

      {/* Tech Stack and Footer */}
      <div
        style={{
          display: "flex",
          justifyContent: "space-between",
          alignItems: "center",
          borderTop: "1px solid rgba(255, 255, 255, 0.08)",
          paddingTop: "20px",
          zIndex: 10,
        }}
      >
        {/* Badges Stack */}
        <div
          style={{
            display: "flex",
            gap: "10px",
            flexWrap: "wrap",
            maxWidth: "700px",
          }}
        >
          {project.stack.slice(0, 4).map((tech) => (
            <div
              key={tech}
              style={{
                display: "flex",
                background: "rgba(255, 255, 255, 0.03)",
                border: "1px solid rgba(255, 255, 255, 0.08)",
                borderRadius: "4px",
                padding: "6px 12px",
                fontSize: 13,
                fontFamily: "monospace",
                color: "#8b93a1",
                fontWeight: 600,
              }}
            >
              {tech}
            </div>
          ))}
          {project.stack.length > 4 && (
            <div
              style={{
                display: "flex",
                background: "rgba(255, 255, 255, 0.03)",
                border: "1px solid rgba(255, 255, 255, 0.08)",
                borderRadius: "4px",
                padding: "6px 12px",
                fontSize: 13,
                fontFamily: "monospace",
                color: "#8b93a1",
                fontWeight: 600,
              }}
            >
              {"+" + (project.stack.length - 4) + " more"}
            </div>
          )}
        </div>

        {/* Footer URL */}
        <div
          style={{
            display: "flex",
            fontSize: 14,
            color: "#8b93a1",
            fontFamily: "monospace",
            fontWeight: 600,
          }}
        >
          {"awssbg-vjit.org/projects/" + slug}
        </div>
      </div>
    </div>,
    { ...size },
  );
}
