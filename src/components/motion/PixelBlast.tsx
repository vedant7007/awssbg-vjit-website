...
// Vendored from ReactBits (github.com/zavalit/bayer-dithering-webgl-demo).
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const animate = () => {
        ...
        if (liquidEffect) {
          const liq = liquidEffect as unknown as {
            uniforms: Map<string, THREE.Uniform>;
          };
          const timeUniform = liq.uniforms.get("uTime");
    ...
    const visObserver = new IntersectionObserver(
      (entries) => {
        for (const e of entries)
          visibilityRef.current.visible = e.isIntersecting;
      },
      { threshold: 0 },
    );
